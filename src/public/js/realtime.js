...
const viewProducts = (productos) => {
    ...
    productos.docs.forEach(item => {
        ...
        <div class="card p-2" style="width: 18rem;">
          <img src=${item.thumbnail} class="card-img-top" height="200" width="150" alt=${item.title}>
          <div class="card-body">
            <h5 class="card-title text-center text-uppercase fs-6 fw-bold">${item.title}</h5>
            <p class="card-title text-center"><span class="fw-semibold"> Categoria: </span> ${item.category}</p>
            <p class="card-title text-center"><span class="fw-semibold"> Stock: </span> ${item.stock}</p>
            <p class="card-title text-center"><span class="fw-semibold"> Precio: </span> $${item.price}</p>
            <div class="d-flex align-items-center mt-3 justify-content-around">
              <button type="button" class="btn-card-danger text-uppercase fw-bold delete-btn" onclick="deleteProduct('${String(item._id)}')">Eliminar</button>
              <button type="button" class="btn-card-ok text-uppercase fw-bold">Editar</button>
            </div>
          </div>
        </div>
        `;
